...
void USI_I2C_Init()
{
  PORT_USI |= (1<<PORT_USI_SDA);  //release SDA
  PORT_USI |= (1<<PORT_USI_SCL);  //release SCL
  USIDR=0xFF;
  USISR = (1<<USISIF) | (1<<USIOIF) | (1<<USIPF) | (1<<USIDC) | (0<<USICNT0);
  USICR = (0<<USISIE) | (0<<USIOIE) | (2<<USIWM0) | (0<<USICS0) | (0<<USICLK) | (0<<USITC);
  DDR_USI |= (1<<PORT_USI_SDA);  //set as output (only after I2C mode has been selected)
  DDR_USI |= (1<<PORT_USI_SCL);  //set as output
/*  TCCR0A = (2<<WGM00);
  OCR0A = 10-1;
